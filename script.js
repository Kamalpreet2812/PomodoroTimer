...
function startBreak()
{    document.getElementById("inner1").style.backgroundImage=" linear-gradient(315deg,orange 0%, #000000 74%)";
    ...
    btimer.innerHTML=checkMinute(min)+":"+checkSeconds(sec);
     t=setTimeout(startBreak,1000);
}
function startTime()
{ document.getElementById("inner1").classList.add("spin1");
...
function checkMinute(min){
    if(min<1 && sec<10)
    document.getElementById("inner1").style.backgroundImage=" linear-gradient(315deg,red 0%, #000000 74%)";
    if(min<10) min="0"+min;
return min;}	
...
document.getElementById("inner1").classList.remove("spin1");
